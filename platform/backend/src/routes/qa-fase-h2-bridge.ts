...
function stagingHost(hostname: string) { return hostname.includes("imobiflow-staging") || hostname === "localhost"; }
...
function adapter(input: Partial<OwnerProviderAdapter> & Pick<OwnerProviderAdapter, "name" | "priorities" | "capabilities">): OwnerProviderAdapter {
  return {
    id: input.name.toLowerCase(), type: "TEST", envNames: [],
    costProfile: { pricingModel: "free", estimatedCost: 0, currency: "BRL", isPaid: false },
    isConfigured: () => true, supports: () => true, normalize: (value) => value,
    healthCheck: async () => "AVAILABLE",
    query: async ({ capabilities }) => ({ status: "success", provider: input.name, consultedAt: new Date().toISOString(), sections: capabilities, values: { full_name: "synthetic" }, warnings: [] }),
    ...input,
  };
}
...
export async function runH2Verification(req: RequestWithAccess) {
  ...
  const owner = await db.propertyOwner.findFirst({ where: { companyId, status: "active", OR: [{ email: { endsWith: "@imobiflow.test" } }, { name: { startsWith: "QA_" } }] }, orderBy: { createdAt: "asc" }, select: { id: true, companyId: true, document: true } });
  const foreign = await db.propertyOwner.findFirst({ where: { companyId: { not: companyId }, status: "active", OR: [{ email: { endsWith: "@imobiflow.test" } }, { name: { startsWith: "QA_" } }] }, orderBy: { createdAt: "asc" }, select: { id: true, companyId: true } });
  if (!owner) return { authenticated: true, owner_found: false, five_xx_count: 0 };
  ...
  let invalidStatus = 0;
  try { await createOwnerProviderOrchestrator().query({ companyId, actorUserId, document: "11222333000180", capabilities: ["COMPANY"] as any, idempotencyKey: "h2-invalid-20260923" }); } catch (error) { invalidStatus = Number((error as { statusCode?: number }).statusCode ?? 0); }
  ...
  return {
    authenticated: true, authorized: true, company_scoped: owner.companyId === companyId,
    ...
    check_count: checks.length, five_xx_count: 0,
  };
}
