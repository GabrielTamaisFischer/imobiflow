...
function OwnerDashboardPage() {
  ...
  const [faseDResult, setFaseDResult] = useState<string | null>(null);
  const [faseDForeignOwnerId, setFaseDForeignOwnerId] = useState("");
  ...
  async function runFaseDProof() {
    const token = getStoredToken();
    if (!token) return;
    const foreignOwnerId = faseDForeignOwnerId.trim();
    if (!foreignOwnerId) return;
    try {
      const result = await apiRequest<Record<string, unknown>>("/internal/qa/fase-d/legal-runtime", {
        method: "POST",
        token,
        body: JSON.stringify({ owner_id: ownerId, foreign_owner_id: foreignOwnerId }),
      });
      setFaseDResult(JSON.stringify(result, null, 2));
    } catch (cause) {
      setFaseDResult(cause instanceof Error ? cause.message : "Falha na prova QA.");
    }
  }
  ...
  return <ModulePage session={session} module={module}>
    ...
    {!dashboard ? <p className="text-sm text-muted-foreground">Proprietário não encontrado.</p> : <>
      ...
      {typeof window !== "undefined" && window.location.hostname.includes("imobiflow-staging") ? <section className="mt-4 rounded-lg border border-dashed border-amber-500/50 bg-amber-50/30 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase text-muted-foreground">QA temporário Fase D</p><p className="text-sm text-muted-foreground">Executa provas autenticadas sanitizadas; remover após a execução.</p></div><div className="flex items-center gap-2"><input aria-label="ID QA da outra empresa" value={faseDForeignOwnerId} onChange={(event) => setFaseDForeignOwnerId(event.target.value)} className="h-9 w-72 rounded-md border border-border bg-background px-2 text-xs" placeholder="ID do Owner QA da outra empresa" /><button type="button" onClick={() => void runFaseDProof()} className="inline-flex h-9 items-center rounded-md border border-border px-3 text-xs font-semibold">Executar prova</button></div></div>{faseDResult ? <pre className="mt-3 max-h-96 overflow-auto rounded-md bg-muted p-3 text-xs">{faseDResult}</pre> : null}</section> : null}
