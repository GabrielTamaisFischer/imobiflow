...
function OwnerDashboardPage() {
  ...
  const [downstream, setDownstream] = useState<Awaited<ReturnType<typeof getOwnerDownstream>>["downstream"] | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      try { setDownstream((await getOwnerDownstream(ownerId)).downstream); } catch { setDownstream(null); }
      try {
  ...
  return <ModulePage session={session} module={module}>
    ...
    {!dashboard ? <p className="text-sm text-muted-foreground">Proprietário não encontrado.</p> : <>
      ...
      {downstream ? <OwnerAutomationPanel downstream={downstream} /> : null}
...
function OwnerAutomationPanel({ downstream }: { downstream: Awaited<ReturnType<typeof getOwnerDownstream>>["downstream"] }) {
  const cards = [
    ["Contrato", downstream.contract.readiness],
    ["Vistoria", downstream.inspection.readiness],
    ["Seguro", downstream.insurance.readiness],
    ["Financiamento", downstream.financing.readiness],
    ["Proposta", downstream.proposal.readiness],
  ] as const;
  const label = (readiness: (typeof cards)[number][1]) => readiness.ready ? "Pronto" : readiness.conflicting_fields.length ? "Conflitos pendentes" : readiness.expired_documents.length ? "Documentos vencidos" : "Dados incompletos";
  return <section className="mt-4 rounded-lg border border-border bg-card p-4" aria-label="Automações Owner 360"><div><p className="text-xs font-semibold uppercase text-muted-foreground">Automações</p><h2 className="text-lg font-semibold">Dados reutilizáveis nos módulos</h2><p className="mt-1 text-xs text-muted-foreground">Readiness factual; não representa aprovação comercial ou jurídica.</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{cards.map(([title, readiness]) => <article key={title} className="rounded-md border border-border p-3"><p className="text-sm font-semibold">{title}</p><p className={`mt-2 text-xs font-semibold ${readiness.ready ? "text-emerald-700" : "text-amber-700"}`}>{label(readiness)}</p>{readiness.missing_fields.length ? <p className="mt-1 text-xs text-muted-foreground">Faltantes: {readiness.missing_fields.join(", ")}</p> : null}{readiness.conflicting_fields.length ? <p className="mt-1 text-xs text-amber-700">Conflitos: {readiness.conflicting_fields.join(", ")}</p> : null}{readiness.expired_documents.length ? <p className="mt-1 text-xs text-destructive">Vencidos: {readiness.expired_documents.join(", ")}</p> : null}</article>)}</div></section>;
}
...
function OwnerProfilePanel({ profile, activeTab, onTabChange, canManage, canViewCredit, canViewLegal, canViewFiscal, busy, onSave }: {
...
function OwnerFiscalViewer({ fiscal }: { fiscal: OwnerFiscalData | null }) {
  const status = fiscal?.status ?? "NOT_CHECKED";
  const labels: Record<string, string> = {
    NOT_CHECKED: "Não consultado",
    NOT_CONFIGURED: "Provider não configurado",
    REGULAR: "Regular",
  ...
  if (!fiscal || status === "NOT_CHECKED" || status === "NOT_CONFIGURED" || status === "NOT_AVAILABLE") {
    return <div className="rounded-md border border-dashed border-border p-4 text-sm text-muted-foreground"><p className="font-medium text-foreground">{labels[status]}</p><p className="mt-1">{status === "NOT_CONFIGURED" ? "Fonte fiscal ainda não configurada." : "Execute uma consulta autorizada para obter dados fiscais."}</p></div>;
  }
  const declarations = Array.isArray(fiscal.declarations) ? fiscal.declarations : [];
  const certificates = Array.isArray(fiscal.certificates) ? fiscal.certificates : [];
  const pendingItems = Array.isArray(fiscal.pending_items) ? fiscal.pending_items : [];
  const text = (row: Record<string, unknown>, key: string) => typeof row[key] === "string" && String(row[key]).trim() ? String(row[key]) : "Não informado";
  const debt = fiscal.active_debt ?? { has_active_debt: null, active_debt_count: null, amount: null, authority: null, reference: null, status: null, checked_at: null };
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Info label="Status da consulta" value={labels[status] ?? status} /><Info label="Situação cadastral" value={fiscal.registration_status} /><Info label="Regularidade tributária" value={fiscal.tax_regular == null ? null : fiscal.tax_regular ? "Regular" : "Irregular"} /><Info label="Fonte" value={fiscal.provider} /></div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Info label="Pendências fiscais" value={fiscal.tax_pending == null ? null : fiscal.tax_pending ? "Existem pendências" : "Nenhuma pendência informada"} /><Info label="Dívida ativa" value={debt.has_active_debt == null ? null : debt.has_active_debt ? "Encontrada" : "Não informada"} /><Info label="Última consulta" value={fiscal.checked_at ? new Date(fiscal.checked_at).toLocaleString("pt-BR") : null} /><Info label="Validade" value={fiscal.expires_at ? new Date(fiscal.expires_at).toLocaleDateString("pt-BR") : null} /></div>
    <section className="rounded-md border border-border p-3"><h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Declarações</h3>{declarations.length ? <div className="space-y-2">{declarations.map((declaration, index) => <article key={`${text(declaration, "year")}-${index}`} className="rounded-md border border-border p-3 text-sm"><p className="font-medium">Exercício {text(declaration, "year")}</p><p className="text-xs text-muted-foreground">Situação: {text(declaration, "status")} · Recebida: {text(declaration, "received_at")}</p></article>)}</div> : <p className="text-sm text-muted-foreground">Nenhuma declaração disponível.</p>}</section>
    <section className="rounded-md border border-border p-3"><h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Certidões fiscais</h3>{certificates.length ? <div className="space-y-2">{certificates.map((certificate, index) => <article key={`${text(certificate, "type")}-${index}`} className="rounded-md border border-border p-3 text-sm"><p className="font-medium">{text(certificate, "type")}</p><p className="text-xs text-muted-foreground">{text(certificate, "issuing_authority")} · {text(certificate, "jurisdiction")} · Resultado: {text(certificate, "result_status")}</p><p className="text-xs text-muted-foreground">Emissão: {text(certificate, "issued_at")} · Validade: {text(certificate, "expires_at")}</p></article>)}</div> : <p className="text-sm text-muted-foreground">Nenhuma certidão disponível.</p>}</section>
    <section className="rounded-md border border-border p-3"><h3 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Pendências e dívida ativa</h3><p className="text-sm">Itens pendentes: {pendingItems.length || (fiscal.tax_pending == null ? "Não informado" : fiscal.tax_pending ? "Há pendências" : "Nenhuma pendência informada")}</p>{debt.has_active_debt === true ? <p className="mt-1 text-sm">Registros de dívida ativa: {debt.active_debt_count ?? "Não informado"}</p> : null}</section>
  </div>;
}
