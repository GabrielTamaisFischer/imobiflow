...
function OwnerDashboardPage() {
  ...
  const [faseDBrokerResult, setFaseDBrokerResult] = useState<string | null>(null);
  ...
  async function runFaseDBrokerProof() {
    const token = getStoredToken();
    if (!token) return;
    const foreignOwnerId = window.prompt("ID QA do proprietário da outra empresa")?.trim();
    if (!foreignOwnerId) return;
    try {
      const result = await apiRequest<Record<string, unknown>>("/internal/qa/fase-d/broker-final", { method: "POST", token, body: JSON.stringify({ owner_id: ownerId, foreign_owner_id: foreignOwnerId }) });
      setFaseDBrokerResult(JSON.stringify(result, null, 2));
    } catch (cause) { setFaseDBrokerResult(cause instanceof Error ? cause.message : "Falha na prova QA."); }
  }
  ...
  return <ModulePage session={session} module={module}>
    ...
    {!dashboard ? <p className="text-sm text-muted-foreground">Proprietário não encontrado.</p> : <>
      ...
      {typeof window !== "undefined" && window.location.hostname.includes("imobiflow-staging") ? <section className="mt-4 rounded-lg border border-dashed border-amber-500/50 bg-amber-50/30 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase text-muted-foreground">QA temporário Fase D</p><p className="text-sm text-muted-foreground">Prova Broker sanitizada; remover após a execução.</p></div><button type="button" onClick={() => void runFaseDBrokerProof()} className="inline-flex h-9 items-center rounded-md border border-border px-3 text-xs font-semibold">Executar prova</button></div>{faseDBrokerResult ? <pre className="mt-3 max-h-96 overflow-auto rounded-md bg-muted p-3 text-xs">{faseDBrokerResult}</pre> : null}</section> : null}
