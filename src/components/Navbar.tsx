    ...
    <header className="relative z-40 border-b border-border/60 bg-background/40 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <a href="#login" className="hidden text-sm text-muted-foreground transition hover:text-foreground sm:block">
            Entrar
          </a>
          <a
            href="#cta"
      ...
      {open && (
        <div className="border-t border-border/60 bg-background/95 px-6 py-4 md:hidden">
          <nav className="flex flex-col gap-3 text-sm">
            ...
            <a href="#cta" onClick={() => setOpen(false)} className="mt-2 rounded-full bg-gradient-brand px-4 py-2 text-center text-sm font-semibold text-primary-foreground">
              Teste grátis
            </a>
          </nav>
        </div>
